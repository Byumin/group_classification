    ...
    try:
        ...
        if not selected_discrete_variable :
            ...
            for iter_num in range(max_iter):
                ...
                #################################################
                # 스쿨프랜즈의 경우
                # 척도의 스케일이 너무 작아서, 학생 이동에 영향을 미치지 못하는 문제가 발생
                # 그렇기 때문에 연속형 비용도 한번 넣어보는 걸로 진행
                # 연속형 편차를 역전시켜 이산형 비용에 더해서 출발 그룹 선정에 반영함 (방향 일치
                #################################################
                # 그룹별 / 모집단 평균 계산
                group_mean = init_grouped_df.groupby("초기그룹")[selected_sort_variable].mean()
                pop_mean = init_grouped_df[selected_sort_variable].mean()
                # 이상치 편차 (제곱)
                diff = pop_mean - group_mean # 이상치보다 많은 경우 음수, 적은 경우 양수
                raw_cont_cost = (diff ** 2).sum(axis=1)  # 그룹별 연속형 편차 (클수록 평균에서 멀다)
                # 최대 편차 값을 기준으로 정규화
                max_cont_cost = raw_cont_cost.max()
                cont_cost = max_cont_cost - raw_cont_cost
                # 그룹별 연속형 가중치 부여
                cont_cost = cont_cost * 5
                # 이산형 불균형 비용 + 연속형 불균형 비용
                for g in group_total_cost:
                    group_total_cost[g] += cont_cost.get(g, 0)
                #################################################
                ...
                # 모든 출발 그룹에 대한 후보군 탐색 완료했을 때
                # 모두 -inf인 경우 처리
                valid_mask = np.isfinite(global_pair_costs)
                if not valid_mask.any():
                    print("유효한 이동 가능 조합 없음. 종료.")
                    return init_grouped_df
                # 모두 -인 경우
                if all(cost <= -50 for cost, _, _, _ in global_pair_costs):
                    print("모든 이동 후보의 비용 개선폭이 음수이므로 종료합니다.")
                    return init_grouped_df
