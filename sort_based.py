...
def get_sortable_method_df(df, selected_sort_variable_dict, group_count, sortable_method, sex_classification, male_class_count, female_class_count):
    '''
    ...
    if sex_classification == '합반':
        # 그룹별로 정렬 및 그룹 할당
        group_dict = {'합반': df}
        for sex, group_df in group_dict.items():
            if sortable_method == 'round-robin':
                group_df = df.sort_values(by=list(selected_sort_variable_dict.keys()), ascending=list(selected_sort_variable_dict.values())).reset_index(drop=True)
                group_df['group'] = None
                for i, idx in enumerate(group_df.index):
                    group = i % group_count
                    group_df.at[idx, 'group'] = group
                group_dict['합반'] = group_df  # 업데이트된 그룹 데이터프레임 저장
        ...
        group_dict = dict(tuple(df.groupby('성별')))
        for sex, group_df in group_dict.items():
            # 그룹별로 정렬 및 그룹 할당
            if sortable_method == 'round-robin':
                group_df = group_df.sort_values(by=list(selected_sort_variable_dict.keys()), ascending=list(selected_sort_variable_dict.values())).reset_index(drop=True)
                group_df['group'] = None
                for i, idx in enumerate(group_df.index):
                    group = i % group_count
                    group_df.at[idx, 'group'] = group
                group_dict[sex] = group_df  # 업데이트된 그룹 데이터프레임 저장
            # serpentine: 그룹을 순차적으로 할당하되, 홀수 그룹은 정방향, 짝수 그룹은 역방향으로 할당 (1->2->3->4->3->2->1)
            elif sortable_method == 'serpentine':
                direction = 1 # 1: 정방향, -1: 역방향
                group = 0 # 초기 그룹 번호
        ...
        # 남여 반 갯수 리스화
        class_count = [male_class_count, female_class_count]
        group_dict = dict(tuple(df.groupby('성별')))
        for (sex, group_df), class_count in zip(group_dict.items(), class_count):
            if sortable_method == 'round-robin':
                group_df = group_df.sort_values(by=list(selected_sort_variable_dict.keys()), ascending=list(selected_sort_variable_dict.values())).reset_index(drop=True)
                group_df['group'] = None
                for i, idx in enumerate(group_df.index):
...
def run(context):
    ...
    full_group_names: list, 그룹 이름의 리스트 (선택적)
    ...
    full_group_names = context.get('full_group_names')
    sex_classification = context.get('sex_classification')
    male_class_count = context.get('male_class_count')
    female_class_count = context.get('female_class_count')

    # * 선택된 정렬 변수에 따라 df를 정렬하고 그룹을 할당
    group_dict = get_sortable_method_df(df, selected_sort_variable_dict, group_count, sortable_method, sex_classification, male_class_count, female_class_count)

    for (group_name, df_sorted), group_count in zip(group_dict.items(), [male_class_count, female_class_count]):
        # 남여 그룹 수가 모두 0인 경우
        # 남학교, 여학교, 합반인 경우 처리
        if male_class_count == 0 and female_class_count == 0:
            group_count = context.get('group_count')
        else:
            pass
        ...
        if selected_discrete_variable:
            ...
            group_dict[group_name] = df_sorted  # 업데이트된 그룹 데이터프레임 저장
    ...
    # 그룹 이름 설정
    if full_group_names:
        if sex_classification == '분반' and male_class_count >= 1 and female_class_count >= 1:
            male_group_names = full_group_names[:male_class_count]
            female_group_names = full_group_names[male_class_count:male_class_count + female_class_count]
            for  (sex, df_sorted), group_names in zip(group_dict.items(), [male_group_names, female_group_names]):
                df_sorted['group'] = df_sorted['group'].apply(lambda x: group_names[x])
                group_dict[sex] = df_sorted  # 업데이트된 그룹 데이터프레임 저장
        elif (sex_classification == '합반' or sex_classification == '남학교' or sex_classification == '여학교') and group_count >= 1:
            for (sex, df_sorted), group_names in zip(group_dict.items(), [full_group_names]):
                df_sorted['group'] = df_sorted['group'].apply(lambda x: group_names[x])
                group_dict[sex] = df_sorted  # 업데이트된 그룹 데이터프레임 저장
        else:
            st.warning("그룹 이름 설정 중 오류가 발생했습니다. 그룹 이름이 설정되지 않습니다.")
    else:
        st.warning("그룹 이름이 설정되지 않았습니다. 확인해주세요.")
    return group_dict