        ...
        for iter_num in range(max_iter_per_col):
            ...
            for source_group in source_groups:
                ...
                for s_idx, s_row in source_students.iterrows():
                    ...
                    # 각 source 학생에 대해 최적의 target_group 찾기
                    for target_group in target_groups:
                        # 이 target_group으로 교환 시 다른 특이분류 컬럼들에 미치는 영향 계산
                        other_cols_penalty = 0
                        for other_col in special_cols:
                            if other_col == col:
                                continue
                            # source 학생이 other_col에도 해당하는지 확인
                            if s_row[other_col]:
                                other_summary = df.groupby('초기그룹')[other_col].sum()
                                current_target_count = other_summary.get(target_group, 0)
                                expected_other = other_summary.sum() / len(other_summary)
                                # target_group에 이 학생이 가면 편차가 얼마나 증가하는지
                                new_deviation = abs((current_target_count + 1) - expected_other)
                                old_deviation = abs(current_target_count - expected_other)
                                other_cols_penalty += max(0, new_deviation - old_deviation)
                        
                        # target_group 내 최적의 target 학생 찾기
                        target_students = df[
                            (df['초기그룹'] == target_group) &
                            (df[col] == False)
                        ]
                        ...
                        for t_idx, t_row in target_students.iterrows():
                            ...
                            total_cost = (w_discrete * disc_cost + 
                                        w_continuous * cont_cost + 
                                        other_cols_penalty * 10)  # 다른 컬럼 균형 페널티 가중치
                            
                            if total_cost < best_cost:
                                best_cost = total_cost
                                best_pair = (s_idx, t_idx)
                                best_target_groups = target_group

                # swap 실행
                if best_pair is None:
                    print(f"[{col}] 더 이상 유사한 학생 쌍 없음 → 종료")
                    continue

                s_idx, t_idx = best_pair
                df.loc[s_idx, '초기그룹'] = best_target_group
                df.loc[t_idx, '초기그룹'] = source_group
                
                print(f"[{col}] Iter {iter_num+1}: {source_group} ↔ {best_target_group} (cost: {best_cost:.2f})")
...
def cost_group_swap_special_v2(max_iter_per_col, w_discrete, w_continuous, init_grouped_df, special_cols, relationship_dict, selected_discrete_variable, selected_sort_variable_dict): #, sex_cls, subject_cls):
    ...
    for col in special_cols:
        ...
        for iter_num in range(max_iter_per_col):
            ...
            counts = dict(zip(summary['초기그룹'], summary[col])) # 0 : 1 이렇게?
            # print("counts >>> ", counts)
            for source_group in source_groups:
                source_students = df[
                    (df['초기그룹'] == source_group) &
...
# cmd log
# ===== [결시생] 유사도 기반 swap 시작 =====
# counts >>>  {0.0: 2, 1.0: 1, 2.0: 4, 3.0: 2, 4.0: 3, 5.0: 2, 6.0: 4}
# 출발학생-도착그룹 : 105162오윤아-1.0
# 출발학생-도착그룹 : 105242김주하-1.0
# 출발학생-도착그룹 : 107272양세이-1.0
# 출발학생-도착그룹 : 107282김보미-1.0
# [결시생] Iter 1: 2.0 ↔ 1.0
# 출발학생-도착그룹 : 105142박성윤-1.0
# 출발학생-도착그룹 : 105051정윤제-1.0
# 출발학생-도착그룹 : 107302이새빛-1.0
# 출발학생-도착그룹 : 107332노태희-1.0
# [결시생] Iter 1: 6.0 ↔ 1.0
# [결시생] 모든 반이 기대값 ±1 이내 → 종료

# ===== [특수학생] 유사도 기반 swap 시작 =====
# counts >>>  {0.0: 3, 1.0: 3, 2.0: 2, 3.0: 2, 4.0: 1, 5.0: 1, 6.0: 1}
# 출발학생-도착그룹 : 101131유시우-5.0
# 출발학생-도착그룹 : 105152이하경-5.0
# 출발학생-도착그룹 : 107312최담희-5.0
# [특수학생] Iter 1: 0.0 ↔ 5.0
# 출발학생-도착그룹 : 105131김연수-5.0
# 출발학생-도착그룹 : 105142박성윤-5.0
# 출발학생-도착그룹 : 105162오윤아-5.0
# [특수학생] Iter 1: 1.0 ↔ 5.0
# counts >>>  {0.0: 2, 1.0: 2, 2.0: 2, 3.0: 2, 4.0: 1, 5.0: 3, 6.0: 1}
# 출발학생-도착그룹 : 105101이은규-4.0
# 출발학생-도착그룹 : 105131김연수-4.0
# 출발학생-도착그룹 : 105152이하경-4.0
# [특수학생] Iter 2: 5.0 ↔ 4.0
# [특수학생] 모든 반이 기대값 ±1 이내 → 종료

###########################################################
# 가장 기본 ver
###########################################################

def cost_group_swap_special_v1(max_iter_per_col, w_discrete, w_continuous, init_grouped_df, special_cols, relationship_dict, selected_discrete_variable, selected_sort_variable_dict): #, sex_cls, subject_cls):
    """
    특이분류학생 균등 배치:
    - 출발/도착 그룹은 편차 기준
    - 학생 교환은 이산형 & 연속형 유사도 최소화 기준
        ...
        for iter_num in range(max_iter_per_col):
            ...
            for source_group in source_groups:
                source_students = df[
                    (df['초기그룹'] == source_group) &
                    (df[col] == True) &
                    # 여기에 관계그룹 0,-1 인 애들 설정 추가
                    (~df['merge_key'].isin(same_class_students))
                ]
...
#         return ['선택과목']
#     elif sex_cls == '합반' and subject_cls == '아니오':
#         return []
...
# # groupby_cols = get_groupby_cols(sex_cls, subject_cls)
# # if groupby_cols:
# #     grouped_iter = init_grouped_df.groupby(groupby_cols)
# # else:
# #     grouped_iter = [(None, init_grouped_df)]
...
#     groupby_cols = get_groupby_cols(sex_cls, subject_cls)
#     if groupby_cols:
#         grouped_iter = df.groupby(groupby_cols)
#     else:
#         grouped_iter = [(None, df)]
...
#             for iter_num in range(max_iter_per_col):
#                 # 반별 편차 계산
#                 summary = group_df.groupby('초기그룹')[col].sum().reset_index()
#                 expected = summary[col].sum() / summary['초기그룹'].nunique()
#                 summary['편차'] = summary[col] - expected
...
#                 # best_pair = None
#                 for source_group in source_groups:
#                     source_students = group_df[
#                         (group_df['초기그룹'] == source_group) &
#                         (group_df[col] == True) &
#                         # 여기에 관계그룹 0,-1 인 애들 설정 추가
#                         (~group_df['merge_key'].isin(same_class_students))
#                     ]
...
#                     for target_group in target_groups:
#                         target_students = group_df[
#                             (group_df['초기그룹'] == target_group) &
#                             (group_df[col] == False)
#                             # 여기에도 다른 관계가 없어야 continue
#                         ]
...
#                     )
#                     # group_df 갱신 (중요!)
#                     group_df = df.loc[group_df.index]
#     return df
...
def cost_group_swap_special_v5(init_grouped_df, special_cols, selected_discrete_variable, selected_sort_variable_dict, w_special=1000, w_discrete=100, w_continuous=1, max_iter_per_col=50):
    df = init_grouped_df.copy()
    cont_vars = list(selected_sort_variable_dict.keys())
    # special_cols = ['특수학생', '운동부', '전출예정', '결시생']
    ...
    for col in special_cols:
        ...
        for iter_num in range(max_iter_per_col):
            ...
            if not target_groups:
                # 특이분류학생 수가 제일 적은 반 설정
                min_value = summary[col].min()
                target_groups = summary.loc[
                    summary[col] == min_value, '초기그룹'
                ].tolist()

                # # 여러 개면 랜덤 선택
                # import random
                # target_groups = [random.choice(target_groups)]
            ...
            for source_group in source_groups:
                for target_group in target_groups:
                    ...
                    for s_idx, s_row in source_candidates.iterrows():
                        for t_idx, t_row in target_candidates.iterrows():

                            # --- 학생 유사도 ---
                            student_dist = compute_student_distance(
                                s_row, t_row,
                            ...
                            before_gap = (
                                abs(summary.loc[
                                    summary['초기그룹'] == source_group, '편차'
                                ].values[0])
                                +
                                abs(summary.loc[
                                    summary['초기그룹'] == target_group, '편차'
                                ].values[0])
                            )
                            ...
                            after_gap = (
                                abs(
                                    (summary.loc[
                                        summary['초기그룹'] == source_group, col
                                    ].values[0] - 1) - expected
                                )
                                +
                                abs(
                                    (summary.loc[
                                        summary['초기그룹'] == target_group, col
                                    ].values[0] + 1) - expected
                                )
                            )

                            special_gain = before_gap - after_gap

                            if special_gain <= 0:
                                continue

                            # --- 기존 분포 손실 ---
                            disc_loss = compute_discrete_loss_after_swap(
                                df, s_idx, t_idx, selected_discrete_variable
                            )

                            cont_loss = compute_continuous_loss_after_swap(
                                df, s_idx, t_idx, cont_vars
                            )

                            total_gain = (
                                w_special * special_gain
                                - w_discrete * disc_loss
                                - w_continuous * cont_loss
                            )
