...
def init_group_assign_v2(tuples, k):
    """
    다단계 정렬(lexsort) → 라운드로빈 방식 초기 그룹 배정

    Parameters
    ----------
    tuples : tuple of np.ndarray
        정렬 기준 변수들 (오른쪽일수록 우선순위 높음)
    k : int
        그룹 수
    -------
    group_assign : np.ndarray
        원본 인덱스 기준 그룹 번호 (0 ~ k-1)
    """
    import numpy as np

    # 1) lexsort로 다단계 정렬 (우선순위 높은 변수가 tuples의 마지막)
    sorted_idx = np.lexsort(tuples)  # 정렬된 인덱스 반환
    n = len(sorted_idx)

    # 2) 정렬된 순서대로 라운드로빈 그룹 배정
    group_assign_sorted = np.zeros(n, dtype=int)

    current_group = 0
    for i, idx in enumerate(sorted_idx):
        group_assign_sorted[i] = current_group
        current_group = (current_group + 1) % k

    # 3) 정렬된 그룹 배정을 원본 인덱스 순서에 맞게 되돌리기
    #    sorted_idx 는 "정렬된 위치 → 원본 인덱스" 매핑이므로
    #    반대로 원본에 재배치해야 함
    group_assign_original = np.zeros(n, dtype=int)
    group_assign_original[sorted_idx] = group_assign_sorted

    return group_assign_original
